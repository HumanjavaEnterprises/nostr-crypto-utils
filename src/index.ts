...
interface CustomCrypto {
  subtle: {
    generateKey(
      algorithm: "Ed25519" | RsaHashedKeyGenParams | EcKeyGenParams | AlgorithmIdentifier,
      extractable: boolean,
      keyUsages: readonly KeyUsage[]
    ): Promise<CryptoKeyPair>;
    generateKey(
      algorithm: Pbkdf2Params | AesKeyGenParams | HmacKeyGenParams | AlgorithmIdentifier,
      extractable: boolean,
      keyUsages: readonly KeyUsage[]
    ): Promise<CryptoKey>;
    digest(algorithm: AlgorithmIdentifier, data: ArrayBufferView): Promise<ArrayBuffer>;
    importKey(
      format: 'raw',
      keyData: Uint8Array,
...
let customCrypto: CustomCrypto;

if (typeof window !== 'undefined' && window.crypto) {
  // Browser environment
  customCrypto = window.crypto as unknown as CustomCrypto;
} else if (typeof globalThis !== 'undefined' && globalThis.crypto) {
  // Node.js environment with crypto
  customCrypto = globalThis.crypto as unknown as CustomCrypto;
} else {
  // Fallback to Node.js webcrypto
  customCrypto = webcrypto as unknown as CustomCrypto;
}

// Ensure crypto is available globally
if (typeof globalThis.crypto === 'undefined') {
  globalThis.crypto = customCrypto as unknown as Crypto;
}
 ...
 * ```
 */
export async function generateKeyPair(seed?: string): Promise<{ privateKey: string; publicKey: string }> {
  let privateKeyBytes;
...
export async function encrypt(
  ...
  const ciphertext = await customCrypto.subtle.encrypt(
    { name: 'AES-CBC', iv },
    key,
    encoded
  );
