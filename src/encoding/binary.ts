 ...
 * @returns Byte array
 */
export function bufferToBytes(buffer: Buffer): Uint8Array {
 ...
 */
export function bytesToNumber(bytes: Uint8Array, littleEndian = false): number {
  let value = 0;
 ...
 */
export function splitBytes(bytes: Uint8Array, chunkSize: number): Uint8Array[] {
...
export function bytesView(
  ...
  return new Uint8Array(bytes.buffer, bytes.byteOffset + (start || 0), 
    (end || bytes.length) - (start || 0));
}
