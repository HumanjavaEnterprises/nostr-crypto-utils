 ...
 */
export function padHex(hex: string, length: number, padEnd = false): string {
  const normalized = normalizeHex(hex);
 ...
 * @returns Hex string
 */
export function numberToHex(num: number, padToBytes?: number): string {
 ...
 */
export function concatenateHex(hexStrings: string[]): string {
 ...
 */
export function splitHex(hex: string, chunkSize: number): string[] {
  const normalized = normalizeHex(hex);
