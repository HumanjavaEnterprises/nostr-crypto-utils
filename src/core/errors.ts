...
export class EncodingError extends NostrError {
  constructor(message: string, code = 'ENCODING_ERROR') {
    super(message, code);
...
export class EventError extends NostrError {
  constructor(message: string, code = 'EVENT_ERROR') {
    super(message, code);
 ...
 */
export function createError(message: string, code: ErrorCode): NostrError {
  switch (code) {
