...
function nprofileEncode(pubkey, relays) {
    ...
    return bech32_1.bech32.encode('nprofile', data, 1000);
}
/**
 ...
 */
function neventEncode(eventId, relays, author, kind) {
    validateHexString(eventId, 64);
    if (relays) {
        relays.forEach(validateRelayUrl);
    }
    ...
    return bech32_1.bech32.encode('nevent', data, 1000);
}
/**
 ...
 * @throws {Error} If parameters are invalid
 */
function naddrEncode(pubkey, kind, identifier, relays) {
    validateHexString(pubkey, 64);
    ...
    return bech32_1.bech32.encode('naddr', data, 1000);
}
/**
 * Encode a relay URL
 * @param url Relay URL
...
function encodeTLV(data) {
    ...
    // pubkey/id for nprofile/nevent. Per NIP-19 there is no separate TLV type 4;
    // the naddr d-tag identifier IS the SPECIAL value.
    const specialBytes = isNaddr
        ? utf8Encoder.encode(data.data)
        : (0, utils_js_1.hexToBytes)(data.data);
        ...
        switch (type) {
            case TLV_TYPES.SPECIAL:
                if (isNaddr) {
                    ...
                    result.data = utf8Decoder.decode(value);
