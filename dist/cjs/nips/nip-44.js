...
function pad(plaintext) {
    ...
    return (0, utils_js_2.concatBytes)(prefix, unpadded, suffix);
}
/**
...
function getConversationKey(privkeyA, pubkeyB) {
    const sharedPoint = secp256k1_js_1.secp256k1.getSharedSecret(privkeyA, (0, utils_js_2.hexToBytes)('02' + pubkeyB));
    const sharedX = sharedPoint.subarray(1, 33);
    return (0, hkdf_js_1.extract)(sha2_js_1.sha256, sharedX, utf8Encoder.encode('nip44-v2'));
}
/**
 * Derive message keys (chacha key, chacha nonce, hmac key) from conversation key and nonce
 */
function getMessageKeys(conversationKey, nonce) {
    const keys = (0, hkdf_js_1.expand)(sha2_js_1.sha256, conversationKey, nonce, 76);
    return {
 ...
 */
function encrypt(plaintext, conversationKey, nonce = (0, utils_js_2.randomBytes)(32)) {
    const { chacha_key, chacha_nonce, hmac_key } = getMessageKeys(conversationKey, nonce);
    const padded = pad(plaintext);
    const ciphertext = (0, chacha_js_1.chacha20)(chacha_key, chacha_nonce, padded);
    const mac = (0, hmac_js_1.hmac)(sha2_js_1.sha256, hmac_key, (0, utils_js_2.concatBytes)(nonce, ciphertext));
    return base_1.base64.encode((0, utils_js_2.concatBytes)(new Uint8Array([2]), nonce, ciphertext, mac));
}
/**
...
function decrypt(payload, conversationKey) {
    ...
    const { chacha_key, chacha_nonce, hmac_key } = getMessageKeys(conversationKey, nonce);
    const expectedMac = (0, hmac_js_1.hmac)(sha2_js_1.sha256, hmac_key, (0, utils_js_2.concatBytes)(nonce, ciphertext));
