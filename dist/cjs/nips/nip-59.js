...
const creation_1 = require("../event/creation");
/** Seal event kind. */
...
async function createRumor(event, senderPubkey) {
    ...
    const id = await (0, creation_1.getEventHash)(base);
    return { ...base, id };
}
/**
...
async function createSeal(rumor, senderPrivateKey, recipientPublicKey) {
    ...
    const ck = (0, nip_44_1.getConversationKey)(privBytes, recipientPublicKey);
    const content = (0, nip_44_1.encrypt)(JSON.stringify(rumor), ck);
    return (0, crypto_1.finalizeEvent)({ kind: exports.KIND_SEAL, content, tags: [], created_at: randomPastTimestamp() }, senderPrivateKey);
}
/**
...
async function createGiftWrap(seal, recipientPublicKey, opts = {}) {
    ...
    const ck = (0, nip_44_1.getConversationKey)((0, utils_js_1.hexToBytes)(ephemeral.privateKey), recipientPublicKey);
    const content = (0, nip_44_1.encrypt)(JSON.stringify(seal), ck);
    const pTag = opts.relayUrl
        ? ['p', recipientPublicKey, opts.relayUrl]
        : ['p', recipientPublicKey];
    return (0, crypto_1.finalizeEvent)({
...
async function unwrapEvent(giftWrap, recipientPrivateKey) {
    ...
    const wrapKey = (0, nip_44_1.getConversationKey)(recipientPrivBytes, giftWrap.pubkey);
    const seal = JSON.parse((0, nip_44_1.decrypt)(giftWrap.content, wrapKey));
    if (seal.kind !== exports.KIND_SEAL)
        throw new Error('gift wrap did not contain a kind 13 seal');
    if (!(await (0, crypto_1.verifySignature)(seal)))
        throw new Error('invalid seal signature');
    const sealKey = (0, nip_44_1.getConversationKey)(recipientPrivBytes, seal.pubkey);
    const rumor = JSON.parse((0, nip_44_1.decrypt)(seal.content, sealKey));
