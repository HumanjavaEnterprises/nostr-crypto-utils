 ...
 */
export function createBunkerURI(remotePubkey, relays, secret) {
    if (!/^[0-9a-f]{64}$/.test(remotePubkey)) {
 ...
 */
export function validateBunkerURI(uri) {
    try {
 ...
 */
export function createSession(remotePubkey) {
    if (!/^[0-9a-f]{64}$/.test(remotePubkey)) {
        throw new Error('remotePubkey must be 64 hex characters');
    }
 ...
 */
export function restoreSession(clientSecretKey, remotePubkey) {
 ...
 */
export function isResponse(payload) {
 ...
 */
export async function wrapEvent(payload, session, recipientPubkey) {
 ...
 */
export function unwrapEvent(event, session) {
    if (event.kind !== 24133) {
 ...
 */
export function connectRequest(remotePubkey, secret, permissions) {
 ...
 */
export function signEventRequest(eventJson) {
 ...
 */
export function nip04EncryptRequest(thirdPartyPubkey, plaintext) {
 ...
 */
export function nip04DecryptRequest(thirdPartyPubkey, ciphertext) {
 ...
 * @param plaintext - Message to encrypt
 */
export function nip44EncryptRequest(thirdPartyPubkey, plaintext) {
 ...
 */
export function createResponseFilter(clientPubkey, since) {
    const filter = {
        kinds: [24133],
        '#p': [clientPubkey],
    };
 ...
 */
export function createRequestFilter(signerPubkey, since) {
    const filter = {
        kinds: [24133],
        '#p': [signerPubkey],
    };
 ...
 */
export function unwrapRequest(event, signerSecretKey) {
    if (event.kind !== 24133) {
        throw new Error(`expected kind 24133, got ${event.kind}`);
    }
 ...
 * @returns Signed kind 24133 event
 */
export async function wrapResponse(response, signerSecretKey, signerPubkey, clientPubkey, conversationKey) {
    const signerSecretKeyBytes = hexToBytes(signerSecretKey);
 ...
 */
export async function handleSignerRequest(request, clientPubkey, handlers, opts) {
